// File: scripts/utils/icons.js
// Used by: scripts/ui/render.js, scripts/ui/header.js, scripts/ui/theme-toggle.js
// Uses: (none)
// Inline SVG path data, keyed by a stable id rather than by
// display text, so icons never break if a label changes.
//
// Every key here is either used directly (sun/moon by the theme
// toggle) or is a plausible icon for a contacts.icon value in
// data/portfolio-data.json (mail/globe/github/linkedin/x/phone/
// briefcase/external) — phone has no current contact using it, but
// is kept as a reasonable option for a future one, unlike the admin-
// only icons (trash/up/down/plus for delete/reorder/add buttons)
// that were removed since this site has no admin UI anymore.
...
export const ICON_LIBRARY = {
    ...
    mail: '<path d="M22 6c0-1.1-.9-2-2-2H4a2 2 0 0 0-2 2v12c0 1.1.9 2 2 2h16a2 2 0 0 0 2-2V6z"></path><path d="M22 6l-10 7L2 6"></path>',
...
export function iconSvg(key) {
    return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${ICON_LIBRARY[key] || ""}</svg>`;
}
